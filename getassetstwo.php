...
if (strlen($SearchTerm) > 0) {
    $SearchQuery = " and ((tribes.meta->>'name' ILIKE '%" . $SearchTerm . "%' or tribes.name ILIKE '%" . $SearchTerm . "%') or (tribes.meta->>'revealed'='true' and (tribes.meta->'reveal_json'->>'name' ILIKE '%" . $SearchTerm . "%'))) ";
}
...
if (strlen($Org) > 0) {
    ...
    if (strlen($Tribe) > 0) {
        $Q = vQ("select count(links) as linked_count, tribes.*, (tribes.meta->>'sales_price')::numeric as price_order from tribes left join links on links.link = tribes.id where " . $TypeQuery . " " . $SearchQuery . " " . $TraitsQuery . " and tribes.meta->>'sales_status' is not null and tribes.chiefs->>'" . $Tribe . "'='chief' and tribes.meta->>'pack_pool' is null and (tribes.meta->>'sales_price' is null or (tribes.meta->>'sales_price' is not null and tribes.meta->>'sales_price' != '')) GROUP BY tribes.id " . $OrderbyQuery . " limit " . $Limit . " offset " . $Offset . ";");
    } else {
